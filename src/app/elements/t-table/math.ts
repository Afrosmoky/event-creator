...
export function calculateTopDotPositions(
	...
	const edgeLength = width;

	const maxDots = Math.floor(edgeLength / minSpacing);
	const minDots = Math.ceil(edgeLength / maxSpacing);

	const totalDots = Math.min(maxDots, Math.max(minDots, 1));

	if (totalDots === 0) return [];

	const positions: [x: number, y: number][] = [];

	const step = 1 / (totalDots + 1); // Wyśrodkowanie

	for (let i = 1; i <= totalDots; i++) {
		const t = i * step;
		positions.push([width * (1 - t), height + offset]); // Krzesła tylko u dołu
	}
