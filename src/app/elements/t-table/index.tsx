...
// @ts-ignore
  ...
  import { MoveController, ResizeController } from '@/app/controllers';
	...
	export interface TTableState extends BaseState, WithSeatsState {
	kind: 't-table';
	config: {
	type: 't-table';
	width: number;
	...
	/**
	* Just to type-gaslight me because I was lazy because first
	* this table needs to be merged with `table` (which is a ROUND table)
	*/
	type: 'circle';
	};
	}
	
	export interface TTableProps extends BaseCanvasProps<TTableState> {}
	
	export function TTable(props: TTableProps) {
        ...
        const resizeController = new ResizeController<{
          ...
          (reload, event, element, zoom, deltaX, deltaY, storage) => {
            ...
            const positionsBottom = calculateBottomDotPositions(
              width,
              height,
              store.spacing - 1,
              store.spacing,
            ...
            const positionsRight = calculateRightDotPositions(
              width,
              height,
              store.spacing - 1,
              store.spacing,
            ...
            const positionsLeft = calculateLeftDotPositions(
              width,
              height,
              store.spacing - 1,
              store.spacing,
            ...
            const positionsTop = calculateTopDotPositions(
              width,
              height * 2,
              store.spacing - 1,
              store.spacing,
            ...
            // Jeśli liczba krzeseł w `storage.chairs` nie pasuje do liczby pozycji, zakończ
            if (storage.chairs.children.length !== allPositions.length) return;
            
            // Iteruj przez wszystkie krzesła i przypisz je do odpowiednich pozycji
            for (const element of storage.chairs.children) {
              const chair = element as SVGGElement;
              const index = +chair.dataset.index!;
            
              // Pobierz pozycję dla danego indeksu
              const [x, y] = allPositions[index];
            
              // Przypisanie pozycji na odpowiednie kwadraty
              if (index < positionsBottom.length) {
                // Pierwszy kwadrat
                  // @ts-ignore
                chair.style.transform = `translate(${x}px, ${y - store.config.height}px)`;
              } else if (index < positionsBottom.length + positionsRight.length) {
                // Drugi kwadrat
                  // @ts-ignore
                chair.style.transform = `translate(${x + store.config.height}px, ${y - store.config.height * 2}px)`;
              } else if (index < positionsBottom.length + positionsRight.length + positionsLeft.length) {
                // Trzeci kwadrat
                  // @ts-ignore
                chair.style.transform = `translate(${x - store.config.height}px, ${y - store.config.height * 2}px)`;
              } else {
