...
import {
  calculateLeftDotPositions,
  calculateBottomDotPositions,
  calculateRightDotPositions,
  calculateTopDotPositions,
} from './math.ts';

export interface UTableState extends BaseState, WithSeatsState {
  kind: 'u-table';
  config: {
    type: 'u-table';
    width: number;
...
export interface UTableProps extends BaseCanvasProps<UTableState> {}
...
// @ts-ignore
      ...
      const width = setAttributeForwawrd(storage.rect, 'width', () => Math.max(40, rawWidth));
      const height = setAttributeForwawrd(storage.rect, 'height', () => Math.max(40, rawHeight));

      if (!reload) {
        mutate(store.config, { width, height });
      }

      storage.anchor.setAttribute('transform', `translate(${width}, -${height})`);
// @ts-ignore
      const offsetX = store.config.height;

      const positionsLeftBottom = calculateBottomDotPositions(width, height, store.spacing - 1, store.spacing)
        .map(([x, y]) => [x - offsetX, y]);

      const positionsRightBottom = calculateBottomDotPositions(width, height, store.spacing - 1, store.spacing)
        .map(([x, y]) => [x + offsetX, y]);

      const positionsRight = calculateRightDotPositions(width, height, store.spacing - 1, store.spacing);
      const positionsLeft = calculateLeftDotPositions(width, height, store.spacing - 1, store.spacing);

      const topFull = calculateTopDotPositions(width, height * 2, store.spacing - 1, store.spacing);
      const positionsTop = topFull.slice(0, Math.ceil(topFull.length / 2));
      const positionsTopBottom = topFull.slice(Math.ceil(topFull.length / 2));
      ...
      store.seats = allPositions.length;

      if (storage.chairs.children.length !== allPositions.length) return;

      const lenRB = positionsRightBottom.length;
      const lenLB = positionsLeftBottom.length;
      const lenR = positionsRight.length;
      const lenL = positionsLeft.length;
      const lenT = positionsTop.length;
        // @ts-ignore
      const lenTB = positionsTopBottom.length;
      ...
      for (const element of storage.chairs.children) {
        ...
        if (index < lenRB) {
            // @ts-ignore
          chair.style.transform = `translate(${x}px, ${y - store.config.height}px)`;
        } else if (index < lenRB + lenLB) {
            // @ts-ignore
          chair.style.transform = `translate(${x}px, ${y - store.config.height}px)`;
        } else if (index < lenRB + lenLB + lenR) {
            // @ts-ignore
          chair.style.transform = `translate(${x + store.config.height}px, ${y - store.config.height * 2}px)`;
        } else if (index < lenRB + lenLB + lenR + lenL) {
            // @ts-ignore
          chair.style.transform = `translate(${x - store.config.height}px, ${y - store.config.height * 2}px)`;
        } else if (index < lenRB + lenLB + lenR + lenL + lenT) {
            // @ts-ignore
  ...
  return (
    ...
    >
      ...
      >
        {/* U-shape table layout */}

        <rect y={// @ts-ignore
            -store.config.height} x={store.config.height} width={store.config.width} height={store.config.height} fill={store.color} ref={ref => resizeController.storage.rect = ref} />
        <rect y={// @ts-ignore
            -store.config.height} x={-store.config.height} width={store.config.width} height={store.config.height} fill={store.color} />
        <rect y={// @ts-ignore
            -store.config.height * 2} width={store.config.width} height={store.config.height} fill={store.color} />
        <rect y={// @ts-ignore
            -store.config.height * 2} x={store.config.height} width={store.config.width} height={store.config.height} fill={store.color} />
        <rect y={// @ts-ignore
            -store.config.height * 2} x={-store.config.height} width={store.config.width} height={store.config.height} fill={store.color} />
          ...
          <For each={Array.from({ length: store.seats }, (_, i) => i)}>
            {n => (
              <g data-index={n}>
                ...
                <text text-anchor="middle" dominant-baseline="middle" font-size="20" fill="#475c6c">
                  {n + 1}
                </text>
              </g>
            )}
        ...
        <g
          ref={ref => resizeController.storage.anchor = ref}
            // @ts-ignore
          transform={`translate(${store.config.width}, -${store.config.height})`}
          on:pointerup={async e => {
              resizeController.handlePointerUp(e);
